...
use \Modules\Administration\Models\Files_follow as FilesModel;

class Files_follow extends ResourceController {
    ...
    public function index() {
        ...
        $sql = "SELECT count(*) as total FROM administration.files_follow ";
        $sql.= "where fol_int_id = '$i' and fxf_date_deletion_date is null";
        ...
        $sql = "SELECT * FROM administration.files_follow where fol_int_id = '$i' ";
        $sql.= "AND fol_int_id = '$i' and fxf_date_deletion_date is null";

        if (strlen($k) >= 3)
            $sql .= " and searchitem like '%$k%' and fxf_date_deletion_date is null";
    ...
    public function create() {
        ...
        $namef = "fxf_text_file";
        $campo = "fol_int_id";
        $table = "administration.files_follow";
        ...
        if ($fil_int_id) {
            $respuesta = [
                'status' => 201,
                'messages' => "Archivo de seguimiento ingresado y almacenado",
                'id' => $fil_int_id
            ];
    ...
    public function delete($id = null) {
        ...
        $existe = $this->files->select('fxf_int_id')->find($id);
        if (!$existe)
            return $this->fail("No existe el archivo: $id", 404);

        if ($this->files->delete($id))
            return $this->respondDeleted(['fxf_int_id' => $id]);
        else
            return $this->failServerError();
    }
}
